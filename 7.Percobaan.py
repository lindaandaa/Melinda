...
class EGM_GUI:
    def __init__(self, root):
        ...
        style = ttk.Style()
        style.configure("TButton", font=("Arial", 14, "bold"), padding=8, background="#81D4FA", foreground="#0D47A1")
        ...
        self.set_axis_button.grid(row=5, column=0, columnspan=2, padx=10, pady=10, sticky="ew")
       
        
        ### Analysis Button (Tombol untuk Analisis sinyal) ###
        self.anal_control_frame = ttk.LabelFrame(self.control_frame, text="Analysis")
        self.anal_control_frame.pack(side=tk.TOP, fill=tk.X, padx=0, pady=(15, 20), anchor="n") 
        ...
        footer_frame.place(relx=1.0, rely=1.0, anchor="se", width=290, height=150, x=23, y=-13)  # Pojok kanan bawah

        ### Condition Frame (Frame untuk kondisi dan status) ###
        condition_frame = ttk.LabelFrame(footer_frame, text="Condition", padding=(15, 10))
        condition_frame.pack(side=tk.RIGHT, padx=15, pady=5)  # Letakkan di kanan dalam footer_frame
        ...
        #Mengatur program agar secara otomatis menyimpan data setiap 3 menit (180.000 milidetik).
    ...
    def reset(self):
        """ Membersihkan data, grafik, dan  dan status kondisi pada GUI """
        # Menghapus data dari grafik pertama dan kedua #
        # Menghentikan animasi terlebih dahulu
        ...
        # Reset sumbu grafik ke nilai default dan kosongkan grafik
        ...
        # Update canvas untuk memperbarui tampilan grafik setelah reset
        self.canvas.draw() 
            ...
            if self.animation_running:  # Mengecek apakah animasi sedang berjalan
                if self.use_random_data:
                    ...
                    try:
                        ...
                        self.stop()  # Menghentikan animasi jika terjadi error
                        return
