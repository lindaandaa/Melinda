...
class EGM_GUI:
    def __init__(self, root):
        ...
        self.start_button.pack(padx=5, pady=5, fill=tk.X) 
        ...
        self.reset_button.pack(padx=5, pady=5, fill=tk.X)
        ...
        self.axis_control_frame.pack(fill=tk.X, pady=(55,50))   # Kurangi padding bawah
        ...
        self.set_axis_button.grid(row=5, column=0, columnspan=4, pady=20, sticky="ew")
        

        
        ### Analysis Button (Tombol untuk Analisis sinyal) ###
        self.anal_control_frame = ttk.LabelFrame(self.control_frame, text="Analysis")
        self.anal_control_frame.pack(side=tk.TOP, fill=tk.X, padx=0, pady=(10, 10), anchor="n") 
        ...
        self.analysis_button.pack(fill=tk.X, pady=30)
        ...
        """### Status Bar Frame (Frame untuk status koneksi) ###
        status_bar_frame = ttk.LabelFrame(footer_frame, text="Connection", 
                                          borderwidth=2, relief="solid", padding=(10, 5))
        status_bar_frame.pack(side=tk.LEFT, padx=20, pady=5, fill=tk.X)
        # Menambahkan label ke status bar dan mengatur font pada label, bukan LabelFrame
        self.connection_label = tk.Label(status_bar_frame, text="Status: Disconnected", font=("Arial", 10), foreground="black", anchor="w")
        self.connection_label.pack(fill=tk.X, padx=10)"""
        ...
        #Misalnya, jika Anda ingin jarak lebih dekat, dapat mengurangi padx menjadi 10
    ...
    def save_image(self):
        ...
        try:
            ...
            # Menyimpan gambar dengan nama file berdasarkan timestamp
            ...
            # Membuka gambar yang telah disimpan dan menambahkan teks
            img = Image.open(filename)
            draw = ImageDraw.Draw(img)
            
            # Menggunakan font tertentu untuk menambahkan teks
            font = ImageFont.truetype("arial.ttf", 14)  # Gunakan font yang sesuai jika diperlukan

            # Menambahkan teks (mean amplitude dan status)
            result_text = self.result_label.cget("text")
            status_text = self.status_label.cget("text")

            # Add text to the image (at position x, y)
            draw.text((10, 10), result_text, font=font, fill="black")  # Menambahkan teks hasil
            draw.text((10, 25), status_text, font=font, fill="black")  # Menambahkan teks status
            ...
            # Menangani error jika terjadi kegagalan dalam menyimpan gambar
    ...
    def save_data(self):
        ...
        try:
            ...
            if filename:
                # Mengambil data dari grafik sinyal EMG dan FFT
                x_data = np.array(self.line1.get_xdata())  # Data waktu dari grafik EMG
                y_data_emg = np.array(self.line1.get_ydata())  # Data sinyal EMG
                y_data_fft = np.array(self.line2.get_ydata())  # Data sinyal FFT
                ...
                # Memeriksa apakah file sudah ada dan menambahkan data baru jika perlu
                if os.path.exists(filename):
                    ...
                    # Jika file belum ada, menyimpan data baru
            ...
            # Menangani error jika terjadi kegagalan dalam menyimpan data
