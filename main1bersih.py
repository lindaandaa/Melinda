...
class EGM_GUI:
    def __init__(self, root):
        ...
        self.paused = False #intialize the pause attribute
    ...
    def animation(self):
        def update_plot():
            if not self.paused: #check in not paused
            ...
            if self.animation_running: #contion if animation is running
    ...
    #coba cobaa masih belum ngebaca
    ...
    def calculate_fft_and_mean(self):
        ...
        if len(self.data1) >= 0:
            ...
            self.line2.set_data(range(len(filter_sig)), filter_sig)
            self.ax2.relim()
            self.ax2.autoscale_view()
            self.canvas.draw()
            self.canvas.flush_events()
    ...
    def toggle_pause(self):
        ...
        if self.paused:
            self.options_menu.entryconfig(3, label="Reseme")
        else:
