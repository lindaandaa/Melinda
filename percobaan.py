...
class EGM_GUI:
    ...
    def calculate_fft_and_mean(self):
        ...
        if len(self.data1) > 0:
            ...
            freq, fft_result = freq[:len(freq)//2], np.abs(fft_result[:len(fft_result)//2])

            # Filter dan batasi frekuensi untuk tampilan lebih jelas
            freq = freq[freq < 100]  # Batasi frekuensi ke 100 Hz untuk tampilan lebih jelas
            fft_result = fft_result[:len(freq)]  # Sesuaikan hasil FFT dengan frekuensi yang dibatasi
