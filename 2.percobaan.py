...
def high_pass_filter(signal, cutoff=5, fs=1000, order=4):
    nyquist = 0.5 * fs
...
class EGM_GUI:
    def __init__(self, root):
        ...
        status_bar_frame = ttk.LabelFrame(footer_frame, text="Koneksi", borderwidth=2, relief="solid", padding=(10, 5))
        ...
        condition_frame = ttk.LabelFrame(footer_frame, text="Kondisi", 
        ...
        condition_label = ttk.Label(condition_frame, text="Nilai: N/A", anchor="e", 
        ...
        status_condition_label = ttk.Label(condition_frame, text="Status Kondisi: Tidak diketahui", 
        ...
        self.ax2.set_xlabel('Frekuensi (Hz)', fontsize=14)
        self.ax2.set_ylabel('Amplitudo (A)', fontsize=14)
        #Ganti Background chart2
        self.line2, = self.ax2.plot([], [], lw=2,color='navy')
    ...
    def calculate_fft_and_mean(self):
        """Hitung FFT, rata-rata, frekuensi puncak, dan tentukan status kelelahan."""
        if len(self.data1) > 0:
            # Konversi data sinyal menjadi array numpy
            signal = np.array(self.data1, dtype=float)
            signal = signal - np.mean(signal)  # Menghilangkan komponen DC
            
            
              # Terapkan high-pass filter dan tampilkan hasil sinyal setelah filter
            filtered_signal = high_pass_filter(signal, cutoff=5, fs=1000, order=4)
            
            
            # Cek apakah sinyal setelah filter masih memiliki data
            if np.all(np.isclose(filtered_signal, 0)):
                print("Filtered signal is all zeros. Check filter parameters.")
                return  # Hentikan jika sinyal kosong setelah filter
                
            
             # Plot sinyal setelah filter pada grafik waktu (untuk memastikan filter bekerja)
            self.line1.set_data(np.arange(len(filtered_signal)), filtered_signal)
            self.ax1.relim()
            self.ax1.autoscale_view()
           ...
           # Ambil hanya frekuensi positif
            ...
            fft_result = np.abs(fft_result[:len(fft_result) // 2])
             ...
             # Filter dan batasi frekuensi untuk tampilan lebih jelas
            freq = freq[freq < 100]  # Batasi frekuensi ke 100 Hz
            fft_result = fft_result[:len(freq)]
            
        
        
            # Plot Update Grafik Sinyal (Grafik 1)
            self.line1.set_data(np.arange(len(signal)), signal)
            self.ax1.relim()
            self.ax1.autoscale_view()

            # Plot Grafik FFT (Grafik 2)
            self.line2.set_data(freq, fft_result)
            self.ax2.relim()
            self.ax2.autoscale_view()
            self.canvas.draw()
            self.canvas.flush_events()

            # Hanya tampilkan frekuensi < 100 Hz pada plot FFT
            self.line2.set_data(freq[freq < 100], fft_result[freq < 100])
            
            # Hitung mean amplitudo, mean frekuensi, dan deteksi puncak frekuensi
            mean_amplitude = np.mean(fft_result)
            peak_frequency = freq[np.argmax(fft_result)]
            mean_frequency = np.sum(freq * fft_result) / np.sum(fft_result)  # Mean frekuensi
            
            # Menentukan status kelelahan berdasarkan ambang mean frequency
            status = "Tidak Kelelahan"
            if mean_frequency < 30:  # Sesuaikan ambang frekuensi ini jika diperlukan
                status = "Kelelahan"
            
            # Update Label Hasil dan Status
            self.result_label.config(text=f"Mean Amplitudo: {mean_amplitude:.2f}, Mean Frequency: {mean_frequency:.2f} Hz")
            self.status_label.config(text=f"Status Kondisi: {status}")



    def analyze_fatigue_status(fft_result, freq):
        """Analisis kelelahan otot berdasarkan mean dan median frequency."""
        mean_frequency = np.sum(freq * fft_result) / np.sum(fft_result)
        median_frequency = freq[np.searchsorted(np.cumsum(fft_result), 0.5 * np.sum(fft_result))]

        # Menentukan status kelelahan
        status = "Tidak Kelelahan"
        if mean_frequency < 30 or median_frequency < 30:  # Threshold 30 Hz sebagai contoh
            status = "Kelelahan"
