...
class EGM_GUI:
    def __init__(self, root):
        ...
        status_label = ttk.Label(status_bar_frame, text="Status: Disconnected", 
                                 font=("Arial", 10), foreground="black", anchor="w")
        status_label.pack(fill=tk.X, padx=10)
        
        # Coba untuk menyambung ke port serial
        ...
        status_condition_label = ttk.Label(condition_frame, text="Status Kondisi: Tidak diketahui", 
                                           anchor="e", font=("Arial", 12), foreground="black")
        ...
        try:
            self.serial_port = serial.Serial('COM7', 9600)  # Sesuaikan dengan port Arduino Anda
        except:
    ...
    """def check_connection(self):
         Fungsi untuk memeriksa koneksi serial (misalnya ke Arduino) 
        try:
            # Coba sambungkan ke port serial (ganti dengan port yang sesuai, misalnya COM3 atau /dev/ttyUSB0)
    ...
    def update_status(self, status):
         Fungsi untuk memperbarui status koneksi pada status bar 
        if status == "Connected":
    ...
    def calculate_fft_and_mean(self):
        ...
        # Hitung statistik FFT
        mean_amplitude = np.mean(fft_magnitude)
        peak_frequency = positive_frequencies[np.argmax(fft_magnitude)]
        mean_frequency = np.sum(positive_frequencies * fft_magnitude) / np.sum(fft_magnitude)
        
        # Tentukan status kelelahan
        status = "Tidak Kelelahan"
        if mean_frequency < 30:
            status = "Kelelahan"
